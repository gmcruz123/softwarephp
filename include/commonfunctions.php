...
function checkTableName($shortTName, $type=false)
{
	...
	if ("docente" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("estado" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("asignatura" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("profesion" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("estudiantes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("habilidades" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Reporte_de_Estudiantes" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("usuarios" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("roles" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Quienes_Somos" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("PROYECTOS" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("PIAR" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("PERFILDOCENTE" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("ESTUDIANTESACARGO" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Estadisticas_de_Estudiantes" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("miscasos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("seguimiento" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("preguntas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("rangocalificacion" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Dicccionario" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
...
{
	$arr = array();
	$strPerm = GetUserPermissions("docente");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="docente";
	}
	$strPerm = GetUserPermissions("estado");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="estado";
	}
	$strPerm = GetUserPermissions("asignatura");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="asignatura";
	}
	$strPerm = GetUserPermissions("profesion");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="profesion";
	}
	$strPerm = GetUserPermissions("estudiantes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="estudiantes";
	}
	$strPerm = GetUserPermissions("habilidades");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="habilidades";
	}
	$strPerm = GetUserPermissions("Reporte de Estudiantes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Reporte de Estudiantes";
	}
	...
	$strPerm = GetUserPermissions("roles");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="roles";
	}
	$strPerm = GetUserPermissions("Quienes Somos");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Quienes Somos";
	}
	$strPerm = GetUserPermissions("PROYECTOS");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="PROYECTOS";
	}
	$strPerm = GetUserPermissions("PIAR");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="PIAR";
	}
	$strPerm = GetUserPermissions("PERFILDOCENTE");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="PERFILDOCENTE";
	}
	$strPerm = GetUserPermissions("ESTUDIANTESACARGO");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="ESTUDIANTESACARGO";
	}
	$strPerm = GetUserPermissions("Estadisticas de Estudiantes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Estadisticas de Estudiantes";
	}
	$strPerm = GetUserPermissions("miscasos");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="miscasos";
	}
	$strPerm = GetUserPermissions("seguimiento");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="seguimiento";
	}
	...
	$strPerm = GetUserPermissions("rangocalificacion");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="rangocalificacion";
	}
	$strPerm = GetUserPermissions("Dicccionario");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Dicccionario";
	}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		$sql = "select ".$gConn->addFieldWrappers( "GroupID" )
			...
			." from ". $gConn->addTableWrappers( "lasierra_ugmembers" )
	...
	$sql = "select ". $gConn->addFieldWrappers( "TableName" )
		...
		." from ". $gConn->addTableWrappers( "lasierra_ugrights" )
...
function AfterFBLogIn( $pUsername, $pPassword, $pDisplayUsername, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("docente")
