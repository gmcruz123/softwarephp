...
class RegisterPage extends RunnerPage
{
	...
	protected function registerNewUser()
	{
		...
		$this->strUsername = $values["CEDULA"];
		$this->strPassword = $values["PASS"];
		...
		$passwordHash = md5( $values["PASS"] );
		$originalpassword = $values["PASS"];

		$retval = DoInsertRecord("docente", $values, $blobfields, $this);
	...
	{
		if( $this->cipherer->isFieldEncrypted("CEDULA") )
			$sUsername = $this->cipherer->MakeDBValue("CEDULA", $strUsername, "", true);	
		else 
			$sUsername = add_db_quotes("CEDULA", $strUsername);
			
		$strSQL = "select count(*) from ". $this->connection->addTableWrappers( "docente" ) 
			. " where " . $this->getFieldSQLDecrypt("CEDULA") . "=" . $sUsername;
	...
	protected function checkIfEmailUnique( $strEmail )
	{
		...
		$strSQL = "select count(*) from ". $this->connection->addTableWrappers( "docente" ) 
			." where ".$this->getFieldSQLDecrypt("")."=".$sEmail;
